    ...
    >
      ...
      {members.length > 0 && (
        <ul className="space-y-1">
          {members.map(m => (
            <li key={m.membershipId} className={`flex justify-between items-center bg-white dark:bg-gray-800 px-3 py-1.5 rounded border ${c.border} text-sm`}>
              <span className="text-gray-800 dark:text-gray-100">{m.name} ({m.email})</span>
