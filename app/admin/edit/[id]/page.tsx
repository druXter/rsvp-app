...
export default async function EditEventPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ shareError?: string }> }) {
  ...
  if (!event || !isOwnerOrAdmin(user, event.ownerId)) {
    return <div className="p-8 dark:text-gray-100">Event nicht gefunden.</div>
  }
        ...
        <form id="edit-event-form" action={updateEvent} className="space-y-4">
          ...
          <ThemeSection color="indigo" title="Gruppen- & Vereins-Features">
            ...
            <div>
              ...
              <input id="eventPin" type="text" name="eventPin" defaultValue={event.eventPin || ''} className={`w-full border ${themeClasses('indigo').border} bg-white dark:bg-gray-800 dark:text-gray-100 p-2 rounded outline-none ${themeClasses('indigo').borderFocus}`} placeholder="z.B. Sommer26 (leer lassen für öffentliches Event)" />
