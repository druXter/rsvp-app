...
export async function GET(
  ...
  // Den Bearbeitungs-Token (falls vorhanden) und die dynamische Basis-URL (origin) aus dem Request auslesen
  const { searchParams, origin } = new URL(request.url)
  const token = searchParams.get('token')

  // Persönlichen Bearbeitungslink generieren und an die Event-Beschreibung anhängen
  const personalLink = token ? `\n\nAntwort nachträglich bearbeiten: ${origin}/${event.slug}?token=${token}` : ''
