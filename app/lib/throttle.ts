...
/**
 * Regeln für einen Passwort-Login: IP-weit großzügiger, pro Konto strenger. `scope` trennt die
 * Zähler der beiden Login-Systeme (Admin 'login', Gäste 'guest-login').
 */
export function loginRules(ip: string, email: string, scope = 'login'): { ip: ThrottleRule; email: ThrottleRule } {
  return {
    ip: { scope: `${scope}:ip`, identifier: ip, limit: 20, windowMs: LOGIN_WINDOW_MS },
    email: { scope: `${scope}:email`, identifier: email, limit: 10, windowMs: LOGIN_WINDOW_MS }
  }
}

/** Regeln für "Passwort vergessen" (jede Anfrage zählt): verhindert, dass jemand fremde Postfächer mit Mails flutet. */
export function resetRules(ip: string, email: string, scope = 'reset'): ThrottleRule[] {
  return [
    { scope: `${scope}:ip`, identifier: ip, limit: 10, windowMs: 60 * MINUTE },
    { scope: `${scope}:email`, identifier: email, limit: 3, windowMs: 60 * MINUTE }
  ]
}

/**
 * Regeln für die öffentliche Registrierung eines Nutzer-Kontos (löst eine Bestätigungs-Mail an eine
 * beliebige Adresse aus): begrenzt, wie viele Mails ein einzelner Rechner bzw. eine einzelne
 * Zieladresse in einer Stunde auslösen kann.
 */
export function registerRules(ip: string, email: string): ThrottleRule[] {
  return [
    { scope: 'register:ip', identifier: ip, limit: 10, windowMs: 60 * MINUTE },
    { scope: 'register:email', identifier: email, limit: 3, windowMs: 60 * MINUTE }
  ]
}

/** Regel für die Passwort-Abfrage bei "Passwort/E-Mail ändern" (Schutz gegen eine gekaperte Sitzung). */
